...
@interface VoxaAudioEngineBridge : NSObject
...
@implementation VoxaAudioEngineBridge {
    voxa::core::VoxaCoreEngine* _cppEngine;
}

- (instancetype)init {
    self = [super init];
    if (self) {
        _cppEngine = new voxa::core::VoxaCoreEngine();
    }
    return self;
}
...
- (NSArray<NSNumber *> *)processAudioBuffer:(const float *)samples count:(int)count {
    ...
    voxa::core::SignalFrame frame = _cppEngine->processAudioBuffer(samples, count);
