...
#include <string>
#include <vector>
#include "voxa_core_engine.h"
#include <android/log.h>

#define LOG_TAG "VoxaJNIBridge"
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__)

static voxa::core::VoxaCoreEngine* g_engineInstance = nullptr;

extern "C" {

JNIEXPORT jboolean JNICALL
Java_com_voxa_VOXA_core_VoxaNativeEngine_nativeInit(JNIEnv *env, jobject thiz, jint sampleRate, jint fftSize) {
    if (g_engineInstance == nullptr) {
        g_engineInstance = new voxa::core::VoxaCoreEngine();
    }
...
Java_com_voxa_VOXA_core_VoxaNativeEngine_nativeProcessAudioFrame(JNIEnv *env, jobject thiz, jfloatArray rawSamples) {
    if (g_engineInstance == nullptr) {
    ...
    voxa::core::SignalFrame frame = g_engineInstance->processAudioBuffer(samplesPtr, len);
...
Java_com_voxa_VOXA_core_VoxaNativeEngine_nativeGetTelemetry(JNIEnv *env, jobject thiz) {
    jdoubleArray result = env->NewDoubleArray(4);
    if (g_engineInstance == nullptr) {
        return result;
    }

    voxa::core::EngineMetrics m = g_engineInstance->getMetrics();
    jdouble buffer[4] = {
        m.cpuLoadPercentage,
...
Java_com_voxa_VOXA_core_VoxaNativeEngine_nativeDestroy(JNIEnv *env, jobject thiz) {
    if (g_engineInstance != nullptr) {
