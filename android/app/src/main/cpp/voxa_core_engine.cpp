...
#define LOG_TAG "VoxaCoreEngineNative"
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__)

namespace voxa {
namespace core {

VoxaCoreEngine::VoxaCoreEngine()
    : m_sampleRate(44100),
      m_fftSize(512),
      m_frameCount(0),
      m_totalAllocatedMemory(1024 * 1024 * 16) {
    m_startTime = std::chrono::high_resolution_clock::now();
    LOGI("VoxaCoreEngine instantiated with 16MB preallocated pool");
}

VoxaCoreEngine::~VoxaCoreEngine() {
    LOGI("VoxaCoreEngine destroyed");
}

void VoxaCoreEngine::initializeEngine(int sampleRate, int fftSize) {
...
void VoxaCoreEngine::applyHammingWindow(float* buffer, size_t size) {
...
void VoxaCoreEngine::computeFFT(float* real, float* imag, size_t size) {
...
SignalFrame VoxaCoreEngine::processAudioBuffer(const float* rawSamples, size_t sampleCount) {
    m_frameCount++;
...
void VoxaCoreEngine::computeMatrixTransform(const float* inputMatrix, float* outputMatrix, int rows, int cols) {
...
EngineMetrics VoxaCoreEngine::getMetrics() {
...
void VoxaCoreEngine::resetState() {
    m_frameCount = 0;
